...
- [ ] `ShaderDef` type: stages, resources, push block, features, hooks.
- [ ] Emit entry points (`vertexMain`/`fragmentMain`/`computeMain`) and stage IO
      with `@position`/`@builtin`/`@flat`.
- [ ] Emit feature selection as spec constants or module choice per the variant
      policy.
- [ ] Keep agent bodies as text injected at hooks, with a source map.
- [ ] Delete `src/shader/assemble.c3`, the marker constants in `load.c3`, and
      the `#define`/`#ifdef` machinery.

**Done when**: a ShaderDef produces a complete module with no markers, no
macros and no `#ifdef`.
